...
def d_dy(f, dy):
    ...
    left3 = (f[:,:,0:1]-8*f[:,:,1:2]+8*f[:,:,3:4]-f[:,:,4:5])/(12*dy)
    left2 = (f[:,:,2:3] - f[:,:,0:1])/(2*dy)
    left1 = (f[:,:,1:2] - f[:,:,0:1])/dy
    
    right1 = (f[:,:,-5:-4]-8*f[:,:,-4:-3]+8*f[:,:,-2:-1]-f[:,:,-1:])/(12*dy)
    right2 = (f[:,:,-1:] - f[:,:,-3:-2])/(2*dy)
    right3 = (f[:,:,-1:] - f[:,:,-2:-1])/dy
    
    return jnp.concatenate([left1,left2,left3,center,right1,right2,right3],axis=2)
